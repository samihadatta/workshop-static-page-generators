...
Unlike modern content management systems (CMS) like Wordpress which dynamically load content from databases and other sources when delivering a webpage to a user, static site generators take the opposite approach: all the webpages of a site are built once changes are made, meaning that page content is not dynamically fetched each time. This allows for an increase in website speed since caching is improved, in addition to improved security since resources (data, images, etc.) need not be acquired from sources outside the scope of the webhost.
